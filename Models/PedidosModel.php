...
/* 
		public function reembolsoPaypal(string $idtransaccion, string $observacion){
			...
			$requestData = $this->select($sql);
			$this->idTransacccion = $idtransaccion;
			if(!empty($requestData)){
				$objData = json_decode($requestData['datospaypal']);
				if (isset($objData->purchase_units[0]->payments->captures[0]->links[1]->href)) {
					...
					var_dump($urlReembolso); die();
				}else{
					$urlReembolso = "https://api.sandbox.paypal.com/v2/payments/captures" .$this->idTransacccion. "/refund";
				}
		...
		public function reembolsoPaypal(string $idtransaccion, string $observacion){
            ...
            if(!empty($requestData)){
                ...
                //$urlReembolso = "https://api.sandbox.paypal.com/v2/payments/captures/4Y280204WG796592B/refund";
                $urlReembolso = $objData->purchase_units[0]->payments->captures[0]->links[1]->href;
                $objTransaccion = CurlConnectionPost($urlReembolso,"application/json",getTokenPaypal());
